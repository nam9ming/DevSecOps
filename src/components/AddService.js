...
function AddService({ open, onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name) { setErrorMsg("잡 이름을 입력하세요."); return; }
    if (!pipelineScript.trim()) { setErrorMsg("파이프라인 스크립트를 입력하세요."); return; }
    ...
    const payload = {
      ...
      pipelineScript,
      parameters: [
        { type: "choice", name: "ENV", description: "배포 대상", choices: ["dev","stage","prod"] }
      ],
    };

    try {
      setLoading(true);
      const res = await axios.post(`${ProxyURL}/api/jobCreate/jobs`, payload, {
      ...
      <div className="w-full max-w-3xl rounded-lg bg-white shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-5">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="ex) shopping-service"
              value={jobName}
              onChange={(e) => setJobName(e.target.value)}
              className="w-full rounded border px-3 py-2"
            />
            <p className="mt-1 text-xs text-gray-500">공백은 자동으로 <code>-</code>로 변환됩니다.</p>
          </div>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-700">
              ...
              <div className="flex gap-2">
                {["dev","stage","prod"].map(v => (
                  <span key={v} className="rounded-full bg-white border px-2 py-1 text-xs">{v}</span>
                ))}
              </div>
            </div>
