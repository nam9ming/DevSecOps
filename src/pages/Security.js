...
export default function Security() {
    ...
    try {
      ...
      if (e?.response?.status === 404) {
        try {
          ...
          setInfo(
            `지정 빌드 #${n}에 JMeter 통계가 없어 최신 빌드 #${fb}로 표시합니다.`
          );
        } catch (e2) {
  ...
  const jReport =
    selJob && selBuild
      ? `${JENKINS}/job/${encodeURIComponent(
          selJob
        )}/${selBuild}/artifact/jmeter_${selBuild}/html/index.html`
      : null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3">
          ...
          >
            <ul className="space-y-1">
              ...
              {!filtered.length && (
                <div className="text-sm text-gray-500">서비스 없음</div>
              )}
            </ul>
          </Panel>
        </div>
                  ...
                  <li key={b.number}>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Badge
                          color={tone(
                            b.result || (b.building ? "BUILDING" : "UNKNOWN")
                          )}
                        >
                          {b.result || (b.building ? "BUILDING" : "UNKNOWN")}
                        </Badge>
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        {b.timestamp
                          ? new Date(b.timestamp).toLocaleString("ko-KR")
                          : "-"}{" "}
        ...
        <div className="lg:col-span-5">
          <Panel title={selBuild ? `JMeter 결과 - #${selBuild}` : "JMeter 결과"}>
            {!selBuild && (
              <div className="text-sm text-gray-500">빌드를 선택하세요.</div>
            )}
            {selBuild && (
