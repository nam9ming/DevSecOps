...
export default function Testing() {
  ...
  const pickBuild = async (n) => {
    ...
    try {
      const [jmres, gateres] = await Promise.all([
        axios.get(`${API}/api/perf/jmeter/summary`, { params: { job: selJob, build: n } }),
        axios.get(`${API}/api/perf/sonar/gate-by-build`, { params: { job: selJob, build: n } })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5">
          <Panel
            title={selBuild ? `결과 - #${selBuild}` : '결과'}
            right={gate?.status ? <Badge color={tone(gate.status)}>Quality Gate: {gate.status}</Badge> : null}
          >
            ...
            {selBuild && (
              <div className="space-y-5">
                ...
                {jm && (
                  <>
                    ...
                    <div className="flex flex-wrap gap-3 mt-2">
                      ...
                      {selJob && selBuild && (
                        <a
                          href={`${JENKINS}/job/${encodeURIComponent(selJob)}/${selBuild}/artifact/sonar-gate.json`}
                          target="_blank"
                          rel="noreferrer"
                          className="underline text-blue-600 text-sm"
                        >
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      ...
                      <div className="p-3 rounded bg-gray-50">
                        ...
                        <div className="text-lg font-semibold">
                          {sonar.coverage ? `${Number(sonar.coverage).toFixed(1)}%` : '-'}
                        </div>
                      </div>
                      <div className="p-3 rounded bg-gray-50">
                        <div className="text-xs text-gray-500">Duplications</div>
                        <div className="text-lg font-semibold">
                          {sonar.dup ? `${Number(sonar.dup).toFixed(1)}%` : '-'}
                        </div>
                      </div>
                    </div>
                  ) : (
                  ...
                  {selJob && (
                    <a
                      href={`http://localhost:9000/dashboard?id=${encodeURIComponent(getSonarKey(selJob))}`}
                      target="_blank"
                      rel="noreferrer"
                      className="underline text-blue-600 text-sm"
                    >
